...
class SocketSignalClient extends NanoresourcePromise {
  ...
  _subscribeMediaStream (peer) {
    const onSignal = (batch) => {
      if (peer.destroyed || !peer.connected) return

      return this.rpc.emit('signal', this._buildMessage({
        remoteId: peer.id,
        topic: peer.topic,
        ...
        process.nextTick(() => peer.emit('stream-error', err))
      })
    }

    peer.subscribeSignal(onSignal)

    return () => peer.unsubscribeSignal(onSignal)
  }
    ...
    this.rpc.actions({
      offer: async (message) => {
        if (message.mediaStream) {
          ...
          message.data.forEach(signal => peer.stream.signal(signal))
          return
        }
  ...
  [kCreatePeer] (opts) {
    ...
    peer.subscribeSignal(async (batch) => {
      if (peer.destroyed || peer.connected) return
