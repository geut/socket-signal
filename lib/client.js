...
class SocketSignalClient extends NanoresourcePromise {
   ...
   * @param {number} opts.timeout How long to wait for processing an incoming peer
  ...
  constructor (socket, opts = {}) {
    ...
    const { id = crypto.randomBytes(32), timeout = 5 * 1000, metadata, simplePeer = {} } = opts

    this.socket = socket
    this.rpc = nanomessagerpc(socket, { timeout })
    this.id = id
  ...
  getPeersByTopic (topic) {
    ...
    return this.peers.filter(peer => peer.topics.find(t => t.equals(topic)))
  }
  ...
  connect (id, topic) {
    ...
    const peer = new Peer({ initiator: true, sessionId: crypto.randomBytes(32), id, topic }, this[kSimplePeer])
  ...
  _buildMessage (data) {
    return { ...data, id: this.id, metadata: this.metadata }
  }
  ...
  async [kOnOffer] (data = {}) {
    ...
    const [answer] = await once(peer, 'ss-peer-answered')
    return this._buildMessage({ remoteId: peer.id, topic: peer.topic, sessionId: peer.sessionId, answer })
  }
  ...
  [kAddPeer] (peer, offer) {
    ...
    const iterator = pEvent.iterator(peer, 'signal', {
      resolutionEvents: ['ss-safe-connect'],
    ...
    try {
      ...
      for await (const signal of iterator) {
        if (signal.type === 'offer') {
          const rpcRequest = this.rpc.call('offer', this._buildMessage({
            remoteId: peer.id,
            topic: peer.topic,
          ...
          peer.emit('ss-peer-answered', signal)
          firstSignal = true
        }
      ...
      if (rpcRequest) rpcRequest.cancel()
      throw err
    }
  }
}
