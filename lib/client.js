...
const kInitializePeer = Symbol('socketsignal.initializepeer')
const kSignalMediaStream = Symbol('socketsignal.signalmediastream')
...
class SocketSignalClient extends NanoresourcePromise {
  ...
  constructor (socket, opts = {}) {
    ...
    this[kSignalMediaStream] = signalMediaStream
    this[kPeers] = new Map()
  ...
  [kCreatePeer] (args) {
    if (typeof this[kSimplePeer] === 'function') {
      return this[kSimplePeer](args, Peer)
    }

    return new Peer(args, this[kSimplePeer])
  }

  [kInitializePeer] (peer) {
    if (peer.destroyed) return

    peer.on('error', err => this.emit('peer-error', err, peer))

    if (this[kSignalMediaStream]) {
      peer.on('signal', data => {
        if (data.type === 'offer') {
