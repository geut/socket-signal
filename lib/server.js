...
class SocketSignalServer extends NanoresourcePromise {
  constructor (opts = {}) {
    ...
    const { onConnect, onDisconnect, onJoin, onLeave, onLookup, onOffer, onCandidates } = opts
  ...
  async addSocket (socket) {
    ...
    const rpc = nanomessagerpc(socket)
    ...
    rpc
      ...
      .then(() => {
        ...
        return this._onConnect(rpc)
      })
      .catch(err => {
        process.nextTick(() => this.emit('connection-error', err))
      })

    rpc.on('rpc-closed', () => {
      log('connection-deleted', rpc.id.toString('hex'))
      this.connections.delete(rpc)
      this._onDisconnect(rpc)
    })
