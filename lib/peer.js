...
const { ERR_PEER_VERSION_INVALID } = require('./errors')

const kInit = Symbol('peer.init')
const kSafeConnected = Symbol('peer.safeconnected')
const kOnSafeConnected = Symbol('peer.onsafeconnected')
...
module.exports = class Peer extends SimplePeer {
  ...
  async waitForConnection () {
    if (this[kSafeConnected]) return
    return pEvent(this, kOnSafeConnected)
  }
  ...
  _onChannelMessage (event) {
    ...
    const type = varint.decode(data, varint.decode.bytes)
    offset += varint.decode.bytes
    data = data.slice(offset)

    if (version !== VERSION) {
      throw new ERR_PEER_VERSION_INVALID(VERSION, version)
    }
  ...
  [kInit] () {
    ...
    this.once(kOnSafeConnected, () => {
      this[kSafeConnected] = true
  ...
  [kSend] (type, chunk) {
    const header = varint.encode(VERSION)
    varint.encode(type, header, varint.encode.bytes)
    this._channel.send(Buffer.concat([Buffer.from(header), chunk], header.length + chunk.length))
  }
