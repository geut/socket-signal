...
const kSendSignal = Symbol('peer.sendsignal')
...
module.exports = class Peer extends NanoresourcePromise {
  constructor (opts = {}) {
    ...
    const { sendSignal, onSignal, initiator, sessionId, id, topic, metadata, localMetadata, subscribeMediaStream, simplePeer = {} } = opts

    assert(sendSignal, 'sendSignal is required')
  ...
  ready () {
    ...
    return pEvent(this.simplePeer, 'connect', {
  ...
  async _open () {
    ...
    this.simplePeer.once('close', () => this.close().catch(() => {}))

    const ready = this.ready()

    const unsubscribe = this[kOnSignal]((peer, candidates) => {
      candidates.forEach(signal => this.simplePeer.signal(signal))
    })
    ...
    const clean = () => {
      unsubscribe()
      this.simplePeer.removeListener('signal', onSignal)
    }
      ...
      limit(async () => {
        ...
        try {
          ...
          if (signal.type === 'offer') {
            const { data, metadata } = await this[kSendSignal](this, 'offer', tmp)
            data.forEach(signal => this.simplePeer.signal(signal))
            this.metadata = metadata
            return
          }

          await this[kSendSignal](this, 'answer', tmp)
        } catch (err) {
          clean()
