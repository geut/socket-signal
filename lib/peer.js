...
module.exports = class Peer extends SimplePeer {
  constructor (data, opts = {}) {
    const { initiator, sessionId, id, topic, metadata } = data
    ...
    this._topics.set(topic.toString('hex'), topic)
  ...
  set metadata (metadata) {
    this[kMetadata] = metadata
    process.nextTick(() => this.emit('metadata-updated', this[kMetadata]))
    return this[kMetadata]
  }
  ...
  waitForConnection () {
    ...
    return once(this, 'ss-peer-added')
  }
  ...
  [kInit] () {
    this.once('ss-peer-added', this[kAddedHandler].bind(this))
    this.once('close', () => process.nextTick(() => this.emit('end')))
    this.on('stream', this[kStreamHandler])
    this.on('track', this[kTrackHandler])
    this.once('connect', () => {
      process.nextTick(() => {
        this.emit('ss-safe-connect')
        this.emit('connect')
        process.nextTick(() => {
